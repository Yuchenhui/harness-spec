...
5. **根据用户选择，生成 verification-specs.md（不修改 OpenSpec 原文件）。**

   **重要原则：不修改 specs.md。** OpenSpec 的文件是 OpenSpec 的领地。

   在 change 目录下创建 `verification-specs.md`，这是 Harness 自己的文件：

   ```markdown
   # Verification Specs: $ARGUMENTS
   # Generated by Harness Spec Reviewer
   # Source: specs.md (original scenarios) + reviewer supplements
   # Initializer reads this file, NOT specs.md

   ## Original Scenarios (from specs.md)
   ...
   - Given email 'test@example.com' and password 'SecurePass123!',
     ...
     [source: specs.md, strengthened by reviewer]

   - Given valid email and password, When POST /auth/register, Then return 201
     [source: specs.md, original]

   ## Supplemented Scenarios (approved by user)

   ### User Registration — Error Cases
   - Given duplicate email 'existing@example.com',
     When POST /auth/register,
     Then response status is 409 and body contains 'already registered'
     [source: reviewer, type: error_case, approved]

   ### User Registration — Edge Cases
   - Given empty email '',
     When POST /auth/register,
     Then response status is 422
     [source: reviewer, type: edge_case, approved]

   ## Unverifiable (downgraded to L5 human review)
   - The login page should look professional
     [source: specs.md, original, verification: L5_human_review]
   ```

   文件结构清晰标注了每条 scenario 的来源：
   - `[source: specs.md, original]` — OpenSpec 原文，未修改
   - `[source: specs.md, strengthened by reviewer]` — OpenSpec 原文被补强（用户确认）
   - `[source: reviewer, type: error_case, approved]` — Reviewer 新增，用户批准
   - `[source: reviewer, type: edge_case, skipped]` — Reviewer 建议但用户跳过（不写入文件）

   Git commit: `git commit -m "harness: generate verification-specs for $ARGUMENTS"`

6. 展示最终确认：

   用 AskUserQuestion：
   ```
   问题: "verification-specs.md 已生成（{n} 条 scenario，其中 {m} 条为补充）。确认进入测试生成阶段？"
   选项:
     - "确认，开始生成测试 (Recommended)"
     - "我想再看看 verification-specs.md"
     - "重新审查 specs"
   ```
...
1. **启动 Initializer subagent 生成验证材料。**
   ...
   **Initializer 读取 verification-specs.md（不是 specs.md）。** 如果 verification-specs.md 不存在（用户跳过了 Phase 0），则 fallback 读 specs.md。
