...
```
/opsx:explore                           ← 可用: deep-research-agent
     ↓
/opsx:new + /opsx:continue              ← 可用: requirements-analyst, system-architect,
     ↓                                          backend-architect, frontend-architect,
     ↓                                          ui-ux-designer
Phase 0: /opsx:review                   ← 自动: spec-reviewer
     ↓                                    可用: openspec-proposal-reviewer, security-engineer
...
Phase 3: /opsx:verify + /opsx:archive   ← 自动: verify 增强版
                                          可用: technical-writer, performance-engineer
```

## 自动调用的 Agent（Harness 内建）

这些 agent 在 `/opsx:apply` 执行时自动启动为 subagent，不需要用户手动调用：

| Agent | 阶段 | 触发条件 | 做什么 |
|-------|------|---------|--------|
| **spec-reviewer** | Phase 0 | `/opsx:review` 或 `/opsx:apply` 开始时 | 审查 specs 质量，交互式补强 |
...
这些 agent 在 `/opsx:continue` 的分步过程中或任何时候按需 `@agent-name` 调用：

### 探索 & 需求阶段

| Agent | 用在哪 | 怎么用 | 价值 |
|-------|--------|--------|------|
| **deep-research-agent** | `/opsx:explore` | `@deep-research-agent 帮我调研 JWT vs Session 的选型` | 系统性调研，多轮推理 |
| **requirements-analyst** | `/opsx:continue` 生成 specs 时 | `@requirements-analyst 帮我从这个需求提取完整的 specs` | 需求→specs 转化，防遗漏 |
| **system-architect** | `/opsx:continue` 生成 design 时 | `@system-architect 审查这个架构设计` | 架构级审查，识别反模式 |
| **openspec-proposal-reviewer** | `/opsx:continue` 生成 proposal 后 | `@openspec-proposal-reviewer 审查这个提案` | 提案质量检查，API 一致性 |

### 设计阶段

| Agent | 用在哪 | 怎么用 | 价值 |
|-------|--------|--------|------|
| **backend-architect** | `/opsx:continue` 生成 design 时 | `@backend-architect 帮我设计这个 API 和数据库` | API 设计、DB schema、安全 |
| **frontend-architect** | `/opsx:continue` 生成 design 时 | `@frontend-architect 帮我设计前端组件架构` | 组件拆分、响应式、性能 |
| **ui-ux-designer** | `/opsx:continue` 生成 design 时 | `@ui-ux-designer 帮我设计这个页面的交互` | UI 规范、设计系统、可访问性 |
...
/opsx:continue                          # 生成 tasks

/opsx:apply                             # Harness 接管
...
```
/opsx:propose "fix-login-bug"           # 一步到位
/opsx:apply                             # Harness 自动完成全部
/git-commit                             # 规范化提交
/opsx:archive
```
